<template>
	<div>
      ...
      <el-row align="middle" type="flex">
        ...
        <el-button
        type="primary"
        @click="newRequest = true">新增请求</el-button></el-col>
      </el-row>   
	...
	<el-table-column
  label="是否需要验证"
  prop="needAuthenticate"></el-table-column>
  <el-table-column
	label="描述"
...
<el-dialog title="新建请求" :visible.sync="newRequest">
  <el-form :model="newData" label-position="left" label-width="80px">
    ...
    <el-form-item label="是否需要验证" placeholder="请选择">
      <el-select v-model="newData.needAuthenticate">
  ...
  <div slot="footer" class="dialog-footer">
    <el-button @click="newRequest = false">取 消</el-button>
    <el-button type="primary" @click="handleNewRequest"
    :loading="isSaving">确 定</el-button>
  </div>
</el-dialog>
<!-- 编辑 -->
<el-dialog title="编辑  区域组-区域" :visible.sync="editRequest">
  <el-form :model="editData" label-position="left" label-width="80px">
    ...
    <el-form-item label="是否需要验证" placeholder="请选择">
      <el-select v-model="editData.needAuthenticate">
        <el-option label="需要" value="1"></el-option>
        <el-option label="不需要" value="0"></el-option>
      </el-select>
    </el-form-item>
  ...
  <div slot="footer" class="dialog-footer">
    ...
    <el-button type="primary" @click="handleEditRequest"
    :loading="isSaving">确 定</el-button>
  </div>
</el-dialog>

<el-dialog title="删除  区域组-区域" :visible.sync="deleteRequest">
  ...
  <div slot="footer" class="dialog-footer">
    <el-button @click="deleteRequest = false">取 消</el-button>
    <el-button type="primary" @click="handleDeleteRequest"
...
<script>
  import { getRequest,addRequest, removeRequest,editRequest } from '../../api/api';
  import {validateLogin} from '../../common/js/validateStatus'
	export default {
        ...
        sourceData: [],
        newData: {
          name: '', needAuthenticate: '', description: ''
        },
        editData: {
        },
        deleteData: {
      ...
      handleNewRequest(){
        ...
        this.isSaving = true;
        let self = this; 
        let Request = {action: this.newData};
      ...
      handleEditRequest(){
        this.isSaving = true;
        let self = this;
        let Request = {action: this.editData };
        editRequest(Request).then(data=>{
          if(!validateLogin(data.data.result)){
            self.$router.push({ path: '/YDManager/login' });
            return;
          }
      ...
      handleDeleteRequest(){
        ...
        let self = this;
        this.deleteData.actionId = this.deleteData.id;
        this.deleteData.deletepwd = this.deleteData.password;
        removeRequest(this.deleteData).then(data=>{
          if(!validateLogin(data.data.result)){
            self.$router.push({ path: '/YDManager/login' });
            return;
          }
          if (data.data.result === 'success') {
            ...
            setTimeout(function(){self.deleteRequest = false}, 2000);
          } else {
            self.$message({
              type: 'error',
              duration: 2000,
      ...
      getRequests(param){
        let self = this;
        getRequest(param).then(function (data) {
          if(!validateLogin(data.data.result)){
            self.$router.push({ path: '/YDManager/login' });
            return;
          }
