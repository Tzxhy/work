<template>
	<div>
      ...
      <el-row align="middle" type="flex">
        <el-col :span="3" :offset="3"><el-input icon="search" type="search" 
        v-model="searchId" 
        placeholder="请输入ID"
        ></el-input></el-col>
        ...
        <el-button
        type="primary"
        @click="newRelationAreaTeam = true">新增  区域组-区域</el-button></el-col>
      </el-row> 
  ...
  import {validateLogin} from '../../common/js/validateStatus'
	export default {
      ...
      queryTableDate(){
        ...
        this.getRelationAreaTeams(query);
      },
      ...
      getRelationAreaTeams(param){
        let self = this;
        getRelationAreaTeam(param).then(function (data) {
          if(!validateLogin(data.data.result)){
            self.$router.push({ path: '/YDManager/login' });
            return;
          }
